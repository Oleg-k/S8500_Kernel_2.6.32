...
static DEVICE_ATTR(set_proximity, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, set_proximity_show, set_proximity_store);
...
static DEVICE_ATTR(set_touchkey_led, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, set_touchkey_led_show, set_touchkey_led_store);
...
//	switch_test = device_create_drvdata(touch_class, NULL, 0, NULL, "switch");
	...
	if (device_create_file(switch_test, &dev_attr_set_proximity ) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_set_proximity.attr.name);
